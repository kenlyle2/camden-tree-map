...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="app.tree" && $field=="cityproperty")
	{
		return 1;
	}
				if($table=="app.tree" && $field=="datelastupdated")
	{
		return now();
	}
				if($table=="app.tree" && $field=="dateofinitialcreationofentry")
	{
		return now();
	}
				if($table=="app.tree" && $field=="floweringtree")
	{
		return 0;
	}
				if($table=="app.tree" && $field=="fruittree")
	{
		return 0;
	}
				if($table=="app.tree" && $field=="lastupdaterid")
	{
		return now();
	}
				if($table=="app.tree" && $field=="privateproperty")
	{
		return 0;
	}
				if($table=="app.tree" && $field=="streettree")
	{
		return 0;
	}
				if($table=="app.tree" && $field=="whenplanted")
	{
		return now();
	}
	return "";
